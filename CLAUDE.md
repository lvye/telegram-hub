...
- `source_connector_state` owns provider-neutral queue claims, scheduling, execution leases, and failure counters. `source_connector_checkpoints` owns provider-specific checkpoints. Cron only claims due connectors and publishes one job per connector; ingestion Queue consumers perform network work.
...
- `migrations/` is the immutable migration history for the sole `telegram-hub-prod` database bound as `DB`.
- Never edit an applied migration. Add a new numbered migration and update schema/index tests.
- Operational source/account rows are production data and must not be committed as migration or seed SQL.
- Destructive schema changes require a verified backup and a deployment sequence that keeps the currently running Worker compatible.
